...
# Weekly Review — направления PRJ-1–PRJ-5
...
Расскажи неделю голосом (1–3 коротких сообщения) или текстом, свободно.
Я разложу сам:

общая картина, деньги, клиенты (PRJ-1–PRJ-2), система и данные (PRJ-3–PRJ-4),
бренд и контент, люди и партнёры, проекты, здоровье, семья, отдых,
обучение, изменения по рискам, ключевые решения, фокус следующей недели.

Не до этого сегодня — просто ответь «пропустим».
```
...
- **Клиенты (PRJ-1–PRJ-2)** — переговоры, первые клиенты, упаковка lean-консалтинга и вывода собственника
- **Система и данные (PRJ-3–PRJ-4)** — интервью и блоки экзоскелета, датасет по рынку продажи бизнесов
- **Бренд и контент** — книга, контент-завод, публикации
- **Команда** — key people movements, hiring, conflicts, mentions of names → roles
- **Проекты** — extract pairs `<имя проекта>: <новый статус / progress note>`. Если user сказал "по экзоскелету — первое интервью сделал" → {name: "PRJ-3. Экзоскелет CEO", status: "active", note: "первое интервью сделано"}
...
*Бизнес:* {x}
*Cashflow:* {x}
*Клиенты (PRJ-1–PRJ-2):* {x}
...
*Проекты обновлены:*
  - PRJ-3. Экзоскелет CEO: active — первое интервью сделано
...
```json
{
  ...
  "team": "...",
  "projects": [
    {"name": "PRJ-3. Экзоскелет CEO", "status": "active", "note": "первое интервью сделано"},
