...
### Доска «Почтовый хаб» (`pochta`)

Всё, что по почте требует рук владельца, живёт карточкой на этой доске, а не в
переписке: отписки со страницей, очистка корзины, подключение ящика, оплата.
Появилось новое такое дело — заводи карточку (`kanban_ops.py --add --board pochta`),
сделано — закрывай. Доска видна в Hermex.

### Утренняя сводка

```bash
… mail_digest.py [--hours 24]
```

Показывает **только важное**: непрочитанное из входящих за сутки по всем
подключённым ящикам. Реклама, рассылки и всё, что ушло бы в «Карантин» или
«Подписки», не перечисляется — считается одной строкой «шум убран» (правило
владельца от 2026-09-26). Нечего сказать — скрипт молчит, утро остаётся тихим.

### Чистка переполненного ящика

```bash
# кто сыплет чаще всего + сколько занято места (для отписки)
… mail_sort_imap.py --report            # папка «Карантин»
… mail_sort_imap.py --report INBOX      # или входящие
# убрать старое из карантина в корзину (восстановимо ~30 дней)
… mail_sort_imap.py --purge --older-than 30 --dry-run
… mail_sort_imap.py --purge --older-than 30
```

Порядок при «ящик переполнен»: разложить → `--report` → владелец отписывается
(в mail.ru есть своя кнопка «Отписаться от рассылок») → `--purge` с примеркой →
боевой прогон. **Чистка = перенос в корзину сервера**, не удаление: если корзины
нет, скрипт отказывается работать. Безвозвратно (`expunge`) не удаляем никогда —
даже по просьбе: сначала корзина, освобождение места идёт уже оттуда самим
владельцем.

### Отписка от рассылок

```bash
… mail_unsubscribe.py --dry-run        # кто попадёт под отписку
… mail_unsubscribe.py                  # отписаться (one-click + письмом)
```

Работает механизмом самих рассылок (`List-Unsubscribe`): один запрос по
стандарту RFC 8058 или письмо на адрес отписки (нужен `MAIL_IMAP_<n>_SMTP_HOST`).
Обычные ссылки автоматически НЕ открываются — такие отдаются владельцу списком.
Никогда: банки, безопасность, документы (`PROTECTED_TOPS`) и папка «Спам».
Каждый адрес пробуется один раз — журнал `$HERMES_HOME/mail/unsubscribed.json`.

### Архив вместо визуального шума

```bash
… mail_sort_imap.py --archive-older 180 [--dry-run]
```

Личные письма из входящих старше N дней, которые не подошли ни одному правилу,
уезжают в одну плоскую папку «Архив». Банки, безопасность и документы остаются
в своих папках — они не архивируются и не чистятся.
