...
|---|---|---|
| `topic` | 4-7 слов суть встречи / заметки | "PRJ-2 — интервью с собственником о выходе из операционки" |
...
**Кнопки черновика (hands-free):** ВСЕГДА заканчивай черновик строкой
`[[draft_actions]]` — гейтвей рендерит кнопки ✅ Сохранить · ✏️ Дополнить · 🗑 Удалить.
Реакция: **«Сохрани текущий черновик…»** → выполни сохранение (Step 4) для этого
черновика; **«Отмени текущий черновик…»** → НЕ сохраняй, ответь «🗑 Черновик отменён»;
дополнение голосом → допиши и снова покажи превью с `[[draft_actions]]`.
