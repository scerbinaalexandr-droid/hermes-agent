...
2. **Контекст = направление.** Сначала ищи пометку направления — она главнее любого имени собственного — и нормализуй к коду:
   - «по lean / лин / производств» → `PRJ-1 Lean`
   - «по собственник / операционк / Пазл / выход из операционки» → `PRJ-2 Собственник`
   - «по экзоскелет / скелет / виртуальн CEO / бюджетирован» → `PRJ-3 Экзоскелет`
   - «по данным / ресерч / Австри / продаж бизнес / датасет» → `PRJ-4 Данные`
   - «финпанель / вайбкод / выписк / OCR» → `PRJ-5 Вайбкодинг`
   - «семья / мама / папа / тётя / бабушк / дедушк / родств / предки / история семьи / летопись» → `PRJ-7 История семьи`
   - книга / контент → `Книга`; квартира Brașov, Pharma RO, здоровье, семья, поездки → как есть
   Если ни пометки, ни имени — `(unspecified)` и в черновике строка *Направление:* `? (PRJ-1…PRJ-7 / личное)` — user уточнит одним словом. Не угадывай.
...
3. **Содержание** — основной message. **Apply privacy guard** из `soul.md`:
   ...
   - незнакомые партнёры и собственники бизнесов → "партнёр X" / "собственник Y"
...
**Кнопки черновика (hands-free):** ВСЕГДА заканчивай черновик строкой `[[draft_actions]]`
(гейтвей рендерит ✅ Сохранить · ➕ Добавить · ✏️ Редактировать · 🗑 Удалить). НЕ пиши
«Подтверди: да/поправь» текстом. Реакция:
- **«Сохрани текущий черновик…»** / «да» / «сохрани» → выполни сохранение (Step 3)
- дополнение голосом (после ➕ Добавить) → допиши и снова покажи с `[[draft_actions]]`
- правка голосом (после ✏️ Редактировать) → измени тип/содержание, снова покажи с `[[draft_actions]]`
- **«Отмени текущий черновик…»** / «отмена» → НЕ сохраняй, ответь «🗑 Черновик отменён»
