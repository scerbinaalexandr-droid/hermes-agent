...
2. **Контекст = направление.** Сначала ищи пометку направления — она главнее любого имени собственного — и нормализуй к коду:
   - «по lean / лин / производств» → `П1 Lean`
   - «по собственник / операционк / Пазл / выход из операционки» → `П2 Собственник`
   - «по экзоскелет / скелет / виртуальн CEO / бюджетирован» → `П3 Экзоскелет`
   - «по данным / ресерч / Австри / продаж бизнес / датасет» → `П4 Данные`
   - «финпанель / вайбкод / выписк / OCR» → `П5 Вайбкодинг`
   - книга / контент → `Книга`; квартира Brașov, Pharma RO, здоровье, семья, поездки → как есть
   Если ни пометки, ни имени — `(unspecified)` и в черновике строка *Направление:* `? (П1–П5 / личное)` — user уточнит одним словом. Не угадывай.
