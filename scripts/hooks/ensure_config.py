...
_SOUL_LANG_MARKER = "ЯЗЫК ОТВЕТА — универсальное правило"
SOUL_LANG_RULE = """

---

## 🗣 ЯЗЫК ОТВЕТА — универсальное правило (всегда русский)

ВСЕ сообщения Александру — на **русском**. Всегда, на каждом ответе, без исключений.

- **НИКОГДА не украинский и не белорусский.** Ни близость языков, ни региональный
  контекст (Молдова, Румыния, СНГ), ни названия месяцев/дней, ни тон входного текста —
  НЕ повод переключаться. Александру это неприятно и читается как поломка.
- **Молдавский / румынский** — только если Александр сам написал тебе на них в этом
  же диалоге. По своей инициативе на другой язык не переходи.
- **Автономный / cron режим** (нет входящего сообщения от Александра — плановый
  ритуал, брифинг, ревью, напоминание): язык всё равно **русский**. Отсутствие его
  реплики НЕ даёт свободы выбора языка — по умолчанию русский.
- Технические термины и идентификаторы (имена проектов, моделей, файлов, вкладок) —
  оставляй на оригинальном языке внутри русского текста.

Самопроверка перед отправкой: если хоть одно слово не русское (и это не тех-термин) —
перепиши на русский до отправки.
"""
...
def _ensure_soul_rule() -> None:
    ...
    try:
        ...
        for marker, block in ((_SOUL_RULE_MARKER, SOUL_EMAIL_RULE),
                              ...
                              (_SOUL_LANG_MARKER, SOUL_LANG_RULE)):
