...
## 2026-06-28 — Universal cross-cutting UX через persona (не по скиллам)

**Контекст:** кнопки черновика подключил per-skill (trip/notes/diary), пропустил /capture → юзер увидел текст вместо кнопок, недоволен. Нужно было «везде одинаково».

**Решение:** глобальное поведенческое правило вшито в persona — `ensure_config.py` идемпотентно дописывает блок в `/opt/data/SOUL.md` на старте (маркер `_SOUL_DRAFT_MARKER`). Persona грузится каждое сообщение → правило применяется ко ВСЕМ скиллам и будущим автоматически. Per-skill маркеры остаются как подкрепление для ключевых.

**Альтернативы:** (а) править каждый SKILL.md — хрупко, пропустишь; (б) repo `memory/soul.md` — НЕ долетает до прода (sticky volume). Отклонены.

**Why:** один источник истины для cross-cutting поведения; не зависит от полноты ручного покрытия.
**How to apply:** любое поведение «делать везде» (кнопки, тон, формат) → через `ensure_config` SOUL-правило, НЕ по скиллам. Прод SOUL = `/opt/data/SOUL.md` (sticky).
**Reversal:** легко (удалить блок + маркер).

---

## 2026-06-28 — Gateway [[marker]] конвенция + детерминированный роутинг кнопок

**Контекст:** Telegram-кнопки нужны hands-free, но `send()` хардкодил `reply_markup=None`; скилл-уровневого пути нет → правка gateway-core (sign-off получен).

**Решение:** маркеры в выводе LLM (как `MEDIA:`): `[[menu_keyboard]]`→ReplyKeyboardMarkup (one_time), `[[draft_actions]]`→InlineKeyboardMarkup (4 кнопки `da:`). Стрипаются в `send()`. Роутинг детерминированный: reply-плитка→label переписывается в точную slash-команду; inline-тап→`da:`-callback инжектит user-ход (черновик в контексте LLM). Всё в `telegram.py` (CLAUDE.md разрешает surgical для CEO-команд), `base.py` не тронут.
...
## 2026-06-30 — Prod-стейт на /opt/data чинится под hermes; ownership self-heal на boot

**Decision:** ВСЕ мутации прод-стейта (cron edit/run, запись файлов, `python --save`) выполняются под пользователем **hermes** (`su -s /bin/sh hermes -c '…'`), НИКОГДА не под root через `railway ssh` (там uid=0). Как safety-net entrypoint (`docker/ceo-os-entrypoint.sh`, секция 2b) на каждом старте self-heal'ит владельца ВСЕГО `/opt/data` (`find … ! -user 10000 -exec chown … `, кроме lost+found). Плюс `get_credentials._persist_refreshed_token` — best-effort (ошибка записи токена логируется, не роняет cron).
**Why:** файл на volume, созданный под root, приложение (hermes, uid 10000) не может перезаписать (а mode 600 — и прочитать). Инцидент 2026-06-30: root-owned `google_token.json` → рушил ВСЕ Google-cron; root-owned `cron/jobs.json` (после моих `cron edit` под root) → шедулер слеп → НИЧЕГО не запускал → оба телефона молчали пол-дня. Это класс ошибки, повторившийся 3× за сессию.
**How to apply:** Любая прод-запись — через `su hermes`. chown/чтение под root — ок (chown это ФИКС). Verify доставку END-TO-END (получение юзером), НЕ верить `last_delivery_error=None`/логам — логи cron-доставку не пишут. Каждый деплой рестартит gateway → доставка во время рестарта теряется. Память: `prod-root-ownership-trap`. Reversal: n/a (это ops-инвариант).


---

## [2026-07-04] Fail-clean: сбои → чистое сообщение юзеру, сырьё только в логи

**Context:** Аудит надёжности (цель 99%) выявил класс: при сбое LLM-провайдера/cron/задачи сырой технический текст (`HTTP 402 Insufficient credits`, Python class-names, timing-диагностика) утекал в чат к CEO — нарушение clean-messages (см. [[clean-messages-no-tech-noise]]). Утечка была в 3+ точках: gateway/run.py (интерактив), cron/scheduler.py (`⚠️ Cron job failed:\n{error}`), run_agent `_emit_auxiliary_failure`.

**Decision:** Единый принцип **fail-clean** на всех user-facing границах доставки: любой сбой → короткое человеческое русское сообщение («Не удалось …, попробуй позже»); полный технический error — ТОЛЬКО в `logger`. Реализация: gateway failed-gate `failed and not already_sent` (+ проброс `failed`/`error` на ВСЕХ return-путях `_run_agent`, иначе gate не срабатывает); cron — clean deliver_content при `not success`; auxiliary-сбои (title-gen) не показываются вообще (нулевое влияние на юзера).

**Why:** CEO читает каждое сообщение; тех-шум неприятен и читается как поломка (прямой приоритет юзера). Actionability для CEO у сырых API-ошибок нулевая — их место в логах + health-monitor (см. [[billing-and-health-monitor]]).

**How to apply:** Новый код, доставляющий текст юзеру, НИКОГДА не вставляет `str(exc)`/`{error}`/HTTP-тела в сообщение. При сбое — clean-фраза + `logger.warning/error` с деталью. Reversal: легко (правки локальны в точках доставки). Codex-reviewed 2026-07-04.

## [2026-09-03] telegram-gateway-rebuild — перестройка шлюза после увольнения

**Контекст.** Владелец ушёл из TANDEM; рабочий номер (и Telegram-аккаунт на нём)
переходит новому сотруднику. Требование: 100% отсутствие доступа с того номера.
Параллельно вскрылось, что бот вообще отключён от Telegram — токен отозван 22.08.

**Альтернативы.**
1. Восстановить прежнего бота, убрав рабочий chat_id из allowlist. Отклонено:
   бот был заведён с рабочего аккаунта — держатель симки мог вернуть себе
   аккаунт (двухэтапная аутентификация там отключена) и вместе с ним бота.
2. Новый бот с тем же именем. Отклонено владельцем: чат на рабочем телефоне
   продолжал бы вести на его действующего бота.
3. **Новый бот с новым именем + удаление старого.** Выбрано.

**Решение.** @Alex21_Assist_bot (имя «Гермес») создан с личного аккаунта; прежний
@Hermes_Alex21_bot удалён владельцем. Allowlist, home-channel и доставка всех
12 cron-джобов собраны заново под chat_id, **подтверждённый живым сообщением**.

**Почему живая проверка.** Первая попытка (23.08) опиралась на запись в памяти и
косвенные признаки — и отключила не тот аккаунт: в доступе остался рабочий, а
владелец оказался заблокирован. Ошибка вскрылась только когда его сообщение
пришло с другого id. Косвенным признакам в вопросах доступа не доверять.

**Сопутствующее.** Отозванный токен ронял весь контейнер (gateway отказывается
стартовать, если ни одна сконфигурированная платформа не подключилась) — теперь
entrypoint снимает токен при 401/404, система деградирует до «без Telegram».

**Reversal cost.** Возврат к прежнему боту невозможен (удалён) и не требуется.
Смена бота — легко: новый токен в ENV, allowlist пересобрать той же процедурой.

## 2026-09-17 — Переоснастка под портфель направлений (после TANDEM)

**Decision.** Память бота (`/opt/data/memory/*.md`, `SOUL.md` база) переписана под ИКИГАЙ и П1–П5; TANDEM — архив только для чтения. Ритуалы `/brief`, `/evening`, `/week` **переиспользованы** (заголовки, примеры, поля недельного обзора `sales/production/marketing → clients/system/brand`), а не написаны заново. Кроны: 3 ритуала переименованы, 7 личных возвращены, 2 мёртвых удалены, «CEO Morning» (личная молитва/фокус) оставлен на паузе до слова владельца.

**Why.** Скиллы читают память структурно (`_lib/memory.py`), TANDEM был только в тексте описаний и примеров — новая система дала бы те же три ритуала с новым риском. Память на проде — источник истины (репо лишь сеет при первом старте), поэтому правки шли на волюм с `.bak-20260917`, репо — зеркало.

**Reversal cost.** Низкая: все файлы с `.bak-20260917` на волюме, `jobs.json.bak-20260917`, бэкап памяти в GitHub 09:25 UTC 16.09; репо — один коммит.

**How to apply.** `capture`/`notes` на проде правятся только напрямую (manifest skip). Новое направление = новый блок `## Пн. …` в `projects.md` + код в списке нормализации `/capture`.

## 2026-09-17 — Служебные тексты ядра: конфиг-флаг вместо правки поведения

**Decision.** Уведомление «Gateway shutting down» гейтится новым ключом `gateway.shutdown_notice_home_channel` (default `true` = upstream), `ensure_config.py` ставит `false`. Текст уведомления — по-русски прямо в `gateway/run.py`. Скрипты `no_agent`-кронов при сбое печатают одну русскую строку и выходят с кодом 0: ненулевой код планировщик оборачивает в технический «watchdog script failed».

**Why.** Домашний чат получал уведомление при каждом деплое даже без активной задачи. Флаг с upstream-дефолтом не ломает тесты и слияния; чат с реальной прерванной задачей по-прежнему предупреждается.

**Reversal cost.** Низкая: убрать ключ из `ensure_config.py` — поведение upstream. Русский текст — одна строка в `run.py`, при слиянии с upstream конфликт очевиден.

## 2026-09-17 — Hermex: сессия 365 дней, keep-list навыков, очередь вместо перебивания

**Decision.** (1) `HERMES_WEBUI_SESSION_TTL=365 дней` — выбор владельца из 30/90/365. (2) Бандл-навыки Hermes отключаются все, кроме keep-list из 8 (`SKILLS_KEEP` в `ensure_config.py`); CEO-навыки не трогаются; список пересчитывается на каждом старте. (3) `display.busy_input_mode=queue`; форк-флаги `display.busy_notice=false`, `display.lifecycle_notices=false`.

**Why.** Сессия 30 дней выкидывала из главного органайзера без предупреждения. 100 чужих навыков (игры, jailbreak, ML) раздували промпт и путали роутинг. Режим interrupt при 36 фото заставил бота 36 раз прервать самого себя, служебные строки о сбоях и очереди — техшум для владельца.

**Reversal cost.** Низкая: TTL — одна переменная в entrypoint; навык вернуть — добавить имя в `SKILLS_KEEP`; флаги — убрать из `ensure_config`, поведение станет upstream.

## 2026-09-18 — Этапы 1–2 мастер-плана: замки и модель

**Decision.** (1) `command_allowlist` очищается на каждом старте (никаких «всегда» для опасных команд), `security.redact_secrets=true`, `updates.pre_update_backup=true`, логи 20MB×5. (2) 8 вспомогательных задач + delegation → `claude-haiku-4-5-20251001` у anthropic; vision — на основной; основная — `claude-sonnet-4-5-20250929`.

**Why.** Пять «вечных» разрешений обходили approval целиком. При `provider: auto` побочные задачи шли в OpenRouter первым (другой вендор; был случай украинского языка). Claude 5 API принимает, но адаптер форка (`4-6`/`4-7` подстроки) и таблица цен её не знают — отчёт о расходах ослеп бы.

**Reversal cost.** Низкая: всё в `ensure_config.py`, один коммит. Переход на Sonnet 5 — после обновления upstream (адаптер + pricing) и живого теста через `hermes chat`.

**Побочное.** `brief` больше не использует `python3 -c` (теперь требует approval / в cron deny) — помощник `skills/ceo/_lib/append_log.py`. Куратор CEO-навыки не трогает: они в `.bundled_manifest` (проверено), pin не нужен.

## 2026-09-18 — Этап 4: субагенты как профили-работники канбана

**Decision.** Четыре именованных работника — отдельные профили Hermes (`/opt/data/profiles/<role>/`): `researcher`, `analyst`, `scribe`, `writer`. Шаблоны правил (SOUL) и конфигов — в `skills/ceo/assign/profiles/`; `provision_profiles.py` переписывает их на каждом старте из entrypoint (память/сессии профилей не трогает). Доска «Поручения» (`assignments`), навык `/assign` ставит карточку с исполнителем и подписывает Telegram владельца на итог (`kanban_notify_subs`). Модель работников — Sonnet 4.5, побочные задачи — Haiku; терминал только у аналитика; `delegation/messaging/cronjob/browser` выключены у всех; 30 минут на поручение; 5 сбоев запуска → блок. Быстрые анонимные помощники (`delegate_task`, Haiku) остаются внутри навыков.

**Why.** Владелец хочет видеть и контролировать «связки» — карточки в Hermex с комментариями работника это дают; `delegate_task` невидим. Профили изолируют память и права по ролям (принципы безопасность/конфиденциальность).
...
## 2026-09-18 — «Зеркало» (Inner Mirror): психоаналитический работник как закрытый профиль

**Контекст.** Владелец пользовался промптом Inner Mirror в ChatGPT; хочет тот же помощник внутри Hermes — и для разборов людей/ситуаций, и для разговора о себе, «в бэкапах и защищён от утечек».

**Решение.** Пятый профиль-работник `mirror` (SOUL — `skills/ceo/assign/profiles/mirror.md`): без web и terminal, собственная закрытая память с профилем владельца (лимиты ×2), своя доска «Зеркало» (`ROLE_BOARD` в `assign.py`). В Telegram уходит только заголовок карточки (уведомление без первой строки summary и без причины блокировки). Живой разговор — в Hermex, профиль `mirror`. В бэкапе профиль (память, история чатов, доска) хранится только как `state/mirror.tar.enc` (AES-256-CBC, PBKDF2 200k, `openssl enc`, ключ `BACKUP_MIRROR_KEY` в Railway Variables); из открытой части бэкапа доска исключена; архив перезаписывается только при изменении (sha256 открытого tar рядом).

**Альтернативы.** (а) Навык в главном ассистенте — отвергнуто: его память и история чатов общие, разговор просочился бы в дневник/таблицы. (б) Не шифровать — репозиторий и так приватный, но это самая чувствительная категория данных; шифр стоит одну переменную.

**Цена отката.** Убрать роль из `ROLES`/`ROLE_BOARD`, функцию `backup_mirror`; данные профиля на волюме остаются. Потеря ключа = архив нечитаем (осознанно).
