...
for mail_script in mail_rules.py mail_sort.py mail_sort_imap.py mail_unsubscribe.py mail_archive_attachments.py mail_digest.py; do
  if [ -f "/opt/hermes/skills/ceo/mail/scripts/$mail_script" ]; then
    ...
    echo "[ceo-os-init] Staged $mail_script → $HERMES_HOME/scripts/$mail_script"
  fi
done
