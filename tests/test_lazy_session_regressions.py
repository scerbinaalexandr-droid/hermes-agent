...
class TestGatewaySurfacesNullResponse:
    ...
    @pytest.mark.asyncio
    async def test_partial_response_surfaces_error(self):
        """Agent returns partial=True with no response → user sees error."""
        from gateway.run import GatewayRunner

        gw = GatewayRunner.__new__(GatewayRunner)
        # The actual test here depends on how _run_agent is structured.
        # We test the response handling logic directly:
        agent_result = {
            "final_response": None,
            "api_calls": 5,
            "partial": True,
        ...
        # This is the logic we're adding:
        if not response and _api_calls > 0:
            _partial = agent_result.get("partial", False)
            _interrupted = agent_result.get("interrupted", False)
            if _partial:
                _error_detail = agent_result.get("error", "processing incomplete")
                response = f"⚠️ Processing stopped: {_error_detail}. Try again."
    ...
    @pytest.mark.asyncio
    async def test_interrupted_response_stays_empty(self):
        ...
        if not response and _api_calls > 0:
            ...
            _interrupted = agent_result.get("interrupted", False)
            if _partial:
                response = f"⚠️ Processing stopped. Try again."
            elif not _interrupted:
